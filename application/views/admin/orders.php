...
<tbody>
 	...
 	foreach($o_data as $key=>$row) 
    {
        ...
        <td>
            ...
            {
            ?>
            <select id="status_<?php    echo $row['o_id']    ?>" style="padding: 4px 10px;border: 1px solid gray;">
                <option value="placed"  <?php if($row['o_status']=="placed"){ echo "selected"; }   ?>>Pending</option>
                <option value="confirmed" <?php if($row['o_status']=="confirmed"){ echo "selected"; }   ?>>Confirmed</option>
                <option value="dispatched" <?php if($row['o_status']=="dispatched"){ echo "selected"; }   ?>>Dispatched</option>
                <option value="delivered" <?php if($row['o_status']=="delivered"){ echo "selected"; }   ?>>Delivered</option>
                <optionvalue="cancelled" <?php if($row['o_status']=="cancelled"){ echo "selected"; }   ?>>Cancelled</option>
                
            </select>
            <button type="button" class="btn btn-sm btn-info" id="btn_<?php echo $row['o_id'];    ?>">Update</button>
            <input type="hidden" id="id_<?php echo $row['o_id'];    ?>"  value="<?php  echo $row['o_key']   ?>" />
            <script>
                $(document).ready(function(){
                    $("#btn_"+"<?php echo $row['o_id'];    ?>").click(function(){
                        var order_id=$("#id_"+"<?php echo $row['o_id'];    ?>").val();
                        var status=$("#status_"+"<?php echo $row['o_id'];    ?>").val();
                        
                        $.post('<?php  echo base_url()  ?>admindashboard/order_update',{order_id:order_id,status:status},function(data,status){
                           alert(data); 
                        });
                    });
                });
        ...
        <td>
            <a href="<?php echo base_url().'admindashboard/orderview/'.$row['o_key']; ?>" class="btn btn-success btn-sm" >
                view
            </a>
        </td>
        </tr>
